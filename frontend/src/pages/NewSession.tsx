...
import { createSession, getResume, saveResume } from "../lib/api";
...
export default function NewSession() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const resume = resumeOverride ? form.resume : savedResume || form.resume;
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-12">
      ...
      <p className="text-gray-400 mb-8">
        Paste the job description or URL and we'll generate tailored prep materials.
      </p>
        ...
        <div>
          ...
          {jdMode === "url" && (
            <p className="text-xs text-gray-500 mt-1">
              We'll fetch and parse the page content. Company & role will be auto-extracted if left blank.
            </p>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-1">
              Interviewer Name <span className="text-gray-500">(optional)</span>
            </label>
            <input
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-1">
              Interviewer Title <span className="text-gray-500">(optional)</span>
            </label>
            <input
              value={form.interviewer_title}
              onChange={set("interviewer_title")}
          ...
          {savedResume && !resumeOverride ? (
            <div className="rounded-lg bg-gray-900 border border-gray-700 px-4 py-3 flex items-center justify-between">
              <div className="min-w-0">
                <span className="text-sm text-green-400 font-medium">Saved resume loaded</span>
