...
function StatCard({ label, value, accent = "text-gray-900 dark:text-white" }: { label: string; value: string | number; accent?: string }) {
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800/50 dark:bg-gray-900/40 dark:shadow-none p-4">
...
function CompetencyBars({ data }: { data: Record<string, number> }) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800/50 dark:bg-gray-900/40 dark:shadow-none p-5">
      <h2 className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider mb-4">Competency Averages</h2>
      <div className="h-64">
...
function ScoreTrend({ data }: { data: { date: string; score: number }[] }) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800/50 dark:bg-gray-900/40 dark:shadow-none p-5">
      <h2 className="text-xs font-semibold text-cyan-600 dark:text-cyan-400 uppercase tracking-wider mb-4">Score Trend</h2>
      <div className="h-52">
        <ResponsiveContainer width="100%" height="100%">
...
export default function Progress() {
  ...
  if (!data || data.sessions_completed === 0) {
    return (
      <div className="mx-auto max-w-4xl px-6 py-10">
        <h1 className="text-2xl font-bold tracking-tight mb-6 text-gray-900 dark:text-white">My Progress</h1>
        <div className="text-center py-20 rounded-2xl border border-dashed border-gray-300 dark:border-gray-800/60 bg-gray-50 dark:bg-gray-900/20">
          ...
          <p className="text-gray-700 dark:text-gray-400 text-lg font-medium mb-2">No scored sessions yet</p>
          <p className="text-gray-600 dark:text-gray-500 text-sm mb-6 max-w-md mx-auto">Complete a role-play session to see your performance tracked here over time.</p>
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-10 space-y-6">
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">My Progress</h1>
        <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Cross-session performance across all your interviews</p>
      </div>
