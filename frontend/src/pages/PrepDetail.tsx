...
export default function PrepDetail() {
  ...
  useEffect(() => {
    ...
    getSession(id).then((s) => {
      setSession(s);
      setLoading(false);
      if (s.mode === "roleplay" && s.status === "awaiting_answer") {
        setTab("Role-Play");
      }
    });
  }, [id]);
  ...
  const analysis = session.analysis as Record<string, string | string[]> | null;
        ...
        <div className="space-y-6">
          {analysis.role_focus && (
            <section>
              ...
              <p className="text-gray-300 bg-gray-900 rounded-lg p-4">{String(analysis.role_focus)}</p>
            </section>
          )}
            ...
            <section>
              ...
              <div className="flex flex-wrap gap-2">
                {(analysis.key_skills as string[]).map((skill, i) => (
                  <span key={i} className="px-3 py-1.5 rounded-full bg-gray-800 text-sm text-gray-200 border border-gray-700">
                    {skill}
                  </span>
                ))}
              </div>
      ...
      {tab === "Q&A" && (
        <div className="space-y-4">
          {session.mode === "prep" && session.answers?.length ? (
            session.answers.map((a, i) => <QuestionCard key={i} answer={a} index={i} />)
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mt-2">
                      <span className="text-xs px-2 py-0.5 rounded bg-gray-800 text-gray-400">{q.category}</span>
                    </div>
                    <p className="text-gray-500 text-sm mt-1">{q.why_asked}</p>
                  </div>
                </div>
              </div>
            ))
